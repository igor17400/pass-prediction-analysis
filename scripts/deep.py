...
def main() -> None:
    torch.manual_seed(0)
    nodes = pl.read_parquet(CACHE / "node_features.parquet")
    passes = pl.read_parquet(CACHE / "pass_features.parquet")
    parts = {s: passes.filter(pl.col("split") == s) for s in ("train", "val", "test")}
    T = {s: build_tensors(nodes, parts[s]) for s in parts}
    Y = {s: torch.from_numpy(parts[s]["completed"].to_numpy().astype(np.float32)) for s in parts}
    dev = device()
    model = PlayerGAT().to(dev)
    ...
    while time.time() - t0 < WALL_S and bad < 3:
        ...
        for i in range(0, len(x), bs):
            idx = perm[i : i + bs]
            loss = lossf(model(x[idx].to(dev), pad[idx].to(dev), p[idx].to(dev)), Y["train"][idx].to(dev))
            opt.zero_grad()
            loss.backward()
            opt.step()
    ...
    pl.concat([parts[s].select("pid", "match_id", "completed", "split").with_columns(p=pl.Series(predict(model, *T[s], dev).astype(np.float64))) for s in parts]).write_parquet(CACHE / "preds_gat.parquet")
    json.dump(dict(n_params=n_params, train_s=train_s, epochs=epoch, best_val_logloss=best, device=str(dev)), open(CACHE / "gat_info.json", "w"), indent=1)
    print(f"{n_params} params, {epoch} epochs, {train_s:.0f}s on {dev}, best val {best:.4f}")
